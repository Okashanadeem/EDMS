...
const OfficerDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats({
        ...
        draft_review_count: drafts.filter(d => d.draft_submitted_at && !d.behalf_approved).length,
  ...
  const statCards = [
    ...
    { label: 'My Active', value: stats.active_count, icon: <Clock size={24} />, color: 'bg-orange-500', path: '/officer/my-documents', desc: 'In progress' },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {statCards.map((card, i) => (
          <Link key={i} to={card.path} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl hover:shadow-slate-200/50 transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest">Recent Official Activity</h3>
            <Link to="/officer/my-documents" className="text-[10px] font-black text-indigo-600 hover:underline flex items-center uppercase tracking-[0.2em]">
              View All Legs <ChevronRight size={14} />
            </Link>
          </div>
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            {loading ? (
              <div className="p-20 text-center text-slate-300 font-bold uppercase tracking-widest animate-pulse">Retrieving Documents...</div>
            ) : stats.recent_docs.length === 0 ? (
              <div className="p-20 text-center text-slate-400 italic font-medium">No recent official correspondence recorded.</div>
            ) : (
              <div className="divide-y divide-slate-50">
                {stats.recent_docs.map((doc) => (
                  <Link 
                    key={doc.id} 
                    to={`/officer/document/${doc.id}`}
                    ...
                    <div className="flex items-center space-x-5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center text-[10px] text-slate-400 mt-1 font-bold uppercase tracking-tighter">
                          <span className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-500 font-mono">{doc.outward_number || doc.inward_number || 'PENDING'}</span>
        ...
        <div className="space-y-6">
          <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest text-center lg:text-left">Review Protocol</h3>
          <div className="bg-slate-900 rounded-[2rem] p-8 text-white shadow-2xl shadow-indigo-200 relative overflow-hidden group">
            <div className="relative z-10">
              <h4 className="font-black text-[10px] uppercase tracking-[0.3em] text-indigo-400 mb-6 border-b border-white/10 pb-4">Standard Operating Procedure</h4>
              <ul className="space-y-6">
                <li className="flex items-start">
                  <div className="bg-white/10 rounded-xl p-1.5 mr-4 mt-0.5 group-hover:scale-110 transition-transform"><CheckCircle size={14} className="text-emerald-400" /></div>
                  <p className="text-xs text-slate-300 leading-relaxed font-medium">
                    Review assistant drafts in the <span className="font-black text-white underline decoration-indigo-500 decoration-2 underline-offset-4">Pending Review</span> section.
                  </p>
                </li>
                <li className="flex items-start">
                  <div className="bg-white/10 rounded-xl p-1.5 mr-4 mt-0.5 group-hover:scale-110 transition-transform"><FileEdit size={14} className="text-amber-400" /></div>
                  <p className="text-xs text-slate-300 leading-relaxed font-medium">
                    If corrections are needed, use the <span className="font-black text-white underline decoration-amber-500 decoration-2 underline-offset-4">Revision Note</span> to instruct your assistant.
                  </p>
                </li>
                <li className="flex items-start">
                  <div className="bg-white/10 rounded-xl p-1.5 mr-4 mt-0.5 group-hover:scale-110 transition-transform"><TrendingUp size={14} className="text-blue-400" /></div>
                  <p className="text-xs text-slate-300 leading-relaxed font-medium">
                    Delegated send allows assistants to dispatch via <span className="font-black text-white underline decoration-blue-500 decoration-2 underline-offset-4">OTP Authorization</span> sent to your mail.
                  </p>
                </li>
              </ul>
            </div>
          ...
          <div className="bg-indigo-50/50 rounded-[2rem] p-8 border border-indigo-100/50 transition-all hover:bg-indigo-50">
            <h4 className="font-black text-indigo-900 text-xs uppercase tracking-widest mb-4">Support & Governance</h4>
            <p className="text-xs text-slate-500 leading-relaxed mb-6 font-medium">
              For role modifications or department access issues, please submit a service request to the Super Admin.
            </p>
            <div className="flex items-center text-[10px] font-black text-indigo-600 uppercase tracking-widest cursor-pointer hover:underline">
              ...
              Audit Full Correspondence History
            </div>
          </div>
        </div>
      </div>
